...
def test_write_scad_contains_all_global_vars():
    ...
    output = _write_to_string(items)
    for var in ('WALL_HEIGHT', 'DOOR_HEIGHT', 'WINDOW_HEADER', 'WINDOW_SILL',
...
def test_write_scad_contains_semantic_modules():
    items = []
    output = _write_to_string(items)
    for mod in ('module wall(', 'module wall_outer(', 'module floor_slab(',
                'module door_wood(', 'module window_standard(', 'module wardrobe('):
